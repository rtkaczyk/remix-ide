...
TxRunner.prototype._executeTx = function (tx, gasPrice, api, promptCb, callback) {
  ...
  if (api.personalMode()) {
    ...
    console.log("Trying to send raw TX")

    const prvKey = Buffer.from('2dde7b79d50730e24f82d426cb359158da58b1e2c0373d4d77916a5d8af606e4', 'hex')
    ...
    const ethTx = new EthJSTX({
      nonce: new BN(nonce),
      gasPrice: new BN(1),
      gasLimit: new BN(1000000), // TODO: estimate gas is used instead of provided value
      to: tx.to,
      value: new BN(tx.value, 10),
    ...
    try {
      executionContext.web3().eth.sendRawTransaction("0x" + serializedTx.toString('hex'), cb)
    } catch (e) {
      return callback(`Send transaction failed: ${e.message} `)
    }
  }
}
...
function run (self, tx, stamp, confirmationCb, gasEstimationForceSend, promptCb, callback) {
  console.log("about to run TX from within TxRunner")
